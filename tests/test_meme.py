...
class MemeTests(BaseTestClass):
    ...
    def test_like_meme(self):
        ...
        with self.logged_in_context(user_id):
            response = self.client.post("/api/toggleLike", data=dict(id=meme_id))
            self.assertEqual(response.status_code, 200)
    ...
    def test_toggleLike_nonexistent_meme(self):
        user_id = self.createTestUser()
        with self.logged_in_context(user_id):
            response = self.client.post("/api/toggleLike", data=dict(id=1))
            self.assertEqual(response.status_code, 404)
    ...
    def test_upload_meme(self):
        ...
        with self.logged_in_context(user_id):
            response = self.client.post(
                "/upload",
                data=dict(meme=meme_file, tags='[{"value": "test meme"}]'),
    ...
    def test_upload_meme_missing_image(self):
        ...
        with self.logged_in_context(user_id):
            response = self.client.post(
                "/upload",
                data=dict(tags='[{"value": "test meme"}]'),
                content_type="multipart/form-data",
            )
            self.assertEqual(response.status_code, 200)
            self.assertIn(
                "Error in the Choose image field - This field is required.",
                response.text,
            )
    ...
    def test_upload_meme_missing_tag(self):
        ...
        with self.logged_in_context(user_id):
            ...
            self.assertEqual(response.status_code, 200)
            self.assertIn(
                "Error in the Tags field - This field is required.", response.text
            )

    def test_upload_meme_non_image(self):
        user_id = self.createTestUser()
        meme_file = FileStorage(
            stream=BytesIO(), content_type="image/jpeg", filename="randomname.jpg"
        )
        ...
        with self.logged_in_context(user_id):
            ...
            self.assertEqual(response.status_code, 200)
            self.assertIn(
                "The uploaded meme is not in a supported format. Please upload a proper image.",
                response.text,
            )
    ...
    def test_delete_meme(self):
        ...
        with self.logged_in_context(user_id):
            response = self.client.post(
                "/delete", data=dict(id=meme_id), content_type="multipart/form-data"
            )
            self.assertEqual(response.status_code, 302)
            self.assertEqual(response.location, "/")
    ...
    def test_delete_meme_invalid_request(self):
        ...
        with self.logged_in_context(user_id):
            response = self.client.post(
                "/delete", data=dict(), content_type="multipart/form-data"
            )
            self.assertEqual(response.status_code, 302)
    ...
    def test_delete_nonexistent_meme(self):
        ...
        with self.logged_in_context(user_id):
            response = self.client.post(
                "/delete", data=dict(id=1), content_type="multipart/form-data"
            )
            self.assertEqual(response.status_code, 302)
            with self.client.session_transaction() as session:
                messages = session["_flashes"]
                self.assertGreater(len(messages), 0)
                self.assertIn(
                    "Unable to locate meme, it may have been deleted.",
                    [x[1] for x in messages],
                )
